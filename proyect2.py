...
class PIF(Model):
    ...
    def receive(self, event):
        if event.getName() == "Mensaje":
            if event.getProfundidad() == 0:
                #print("Ya se recorrio la profundidad que dieron al inicio")
                if self.visited == False:
                    self.visited = True
                    self.padre = event.getSource()
                """if self.padre != self.id and self.banderaMensaje == False:
                    self.banderaMensaje = True
                    newevent = Mensaje("Mensaje", self.clock+1.0, self.padre, self.id, event.profundidad)
                    print (" Soy el nodo ", self.id, " y REGRESO a mi PADRE ", self.padre, " en el tiempo ", newevent.getTime(), " la profundida es ", newevent.getProfundidad())
            ...
            if event.getProfundidad() > 0:
                ...
                if self.visited == False:
                    ...
                    profundidad = event.getProfundidad() - 1
                    for i in self.sinVisitar:                               #Manda mensaje a todos sus vecinos menos a su padre
                        newevent = Mensaje("Mensaje", self.clock+1.0, i, self.id, profundidad)
                        print (" Soy el nodo ", self.id, " y mando MENSAJE a mi vecino: ", i, " en el tiempo ", newevent.getTime(), " la profundida es ", profundidad )
                        self.transmit(newevent)
            #print("Soy el nodo ", self.id, "recibo la profundidad ", event.profundidad)
            ...
            for r in range(len(self.neighbors)):                #Reviso si todos los vecinos ya tienen verdadero en su estado de recibi
                if self.recibi[r] == False:
                    alerta = False
            if alerta == True or event.getProfundidad() == 0:                              #Si ya es verdadera toda su lista de vecinos y el padre es diferente del que soy mando mensaje
                if self.padre != self.id:
                    newevent = Mensaje("Mensaje", self.clock+1.0, self.padre, self.id, event.profundidad)
                    print (" Soy el nodo ", self.id, " y mando REGRESA a mi PADRE ", self.padre, " en el tiempo ", newevent.getTime())
                    self.transmit(newevent)
...
prof = int(input())
seed = Mensaje("Mensaje", 0.0, 1, 1, prof)
